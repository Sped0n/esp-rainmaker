...
#if ESP_IDF_VERSION >= ESP_IDF_VERSION_VAL(6, 0, 0)
            /* Detect the key type from its PSA attributes. mbedtls_pk_can_do_psa()
             * is unsuitable here: it returns 0 both for an uninitialised context
             * and when the parsed key's PSA algorithm policy does not match the
             * exact alg queried, so it misreports valid keys. */
            int is_ec = 0, is_rsa = 0;
            psa_key_attributes_t key_attr = PSA_KEY_ATTRIBUTES_INIT;
            if (mbedtls_pk_get_psa_attributes(&claim_data->key, PSA_KEY_USAGE_SIGN_HASH, &key_attr) == 0) {
