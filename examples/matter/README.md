...
- ESP-IDF v5.4 or later, according to each example's `idf_component.yml`.
- ESP RainMaker SDK checkout containing these examples.

ESP-Matter is pulled by the ESP-IDF Component Manager as the `espressif/esp_matter` managed component. You no longer need to clone or export a separate ESP-Matter checkout for these examples.

If you need a local ESP-Matter checkout for development, override the component path in the example's `main/idf_component.yml`:
