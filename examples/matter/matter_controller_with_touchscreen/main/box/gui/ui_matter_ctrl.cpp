...
static lv_obj_t *g_card_list = NULL;
...
static lv_coord_t g_saved_scroll = 0;
...
static void save_scroll_locked(void)
{
    if (g_card_list) {
        g_saved_scroll = ui_main_get_layout()->is_large
                                 ? lv_obj_get_scroll_y(g_card_list)
                                 : lv_obj_get_scroll_x(g_card_list);
    }
}
...
static void rebuild_device_list_locked(bool save_current_scroll)
{
    ...
    ui_list_device();
    lv_obj_update_layout(g_card_list);
    if (ui_main_get_layout()->is_large) {
        lv_obj_scroll_to_y(g_card_list, g_saved_scroll, LV_ANIM_OFF);
    } else {
        lv_obj_scroll_to_x(g_card_list, g_saved_scroll, LV_ANIM_OFF);
    }
}
...
static void create_refresh_button(void)
{
    if (!g_refresh_btn) {
        ...
        lv_obj_set_size(g_refresh_btn, ui_layout_scale(btn_return_width), ui_layout_scale(btn_return_width));
        ...
        lv_obj_set_style_text_font(g_refresh_label, layout->body_font, LV_STATE_DEFAULT);
...
static void ui_list_device(void)
{
    ...
    for (int online_pass = 1; online_pass >= 0; --online_pass) {
        for (size_t i = 0; i < device_count; ++i) {
            matter_device_vm_item_t *item = &s_device_snapshot[i];
            if (item->device_type == MATTER_DEVICE_TYPE_UNKNOWN || item->is_online != (online_pass != 0)) {
                continue;
            }
            lv_obj_t *g_func_btn = lv_btn_create(g_card_list);
            ...
            if (item->is_online) {
                ...
                device_click_target_t *target =
                        (device_click_target_t *)ui_calloc(1, sizeof(device_click_target_t));
                if (target) {
    ...
    if (0u == kind_to_show) {
        if (!g_hint_label) {
            ...
            lv_obj_set_style_text_font(g_hint_label, layout->body_font, LV_STATE_DEFAULT);
            lv_obj_align(g_hint_label, LV_ALIGN_CENTER, 0, ui_layout_scale(hint_align_y));
        }
...
void ui_matter_device_state_update(uint64_t node_id, uint16_t endpoint_id)
{
    ...
    if (card) {
        bool online_changed = card->is_online != item.is_online;
        update_device_card_locked(card, &item);
        if (online_changed) {
            uint32_t online_peer_count = 0;
            for (size_t i = 0; i < s_device_card_count; ++i) {
                if (&s_device_cards[i] != card && s_device_cards[i].is_online) {
                    ++online_peer_count;
                }
            }
            lv_obj_move_to_index(card->button, online_peer_count);
        }
    }
    ui_release();
}
...
static void clean_screen_with_button_locked(void)
{
    ...
    const ui_layout_t *layout = ui_main_get_layout();
    lv_coord_t list_x = layout->is_large ? ui_layout_scale(8) : 0;
    lv_coord_t list_y = layout->is_large ? ui_layout_scale(32) : control_button_first_row;
    lv_coord_t list_bottom = layout->is_large ? ui_layout_scale(8) : 0;
    g_card_list = lv_obj_create(g_page);
    lv_obj_set_pos(g_card_list, list_x, list_y);
    lv_obj_set_size(g_card_list, layout->page_width - (2 * list_x),
                    layout->page_height - list_y - list_bottom);
    lv_obj_set_style_bg_opa(g_card_list, LV_OPA_TRANSP, LV_PART_MAIN);
    lv_obj_set_style_border_width(g_card_list, 0, LV_PART_MAIN);
    lv_obj_set_style_radius(g_card_list, 0, LV_PART_MAIN);
    lv_obj_set_style_pad_all(g_card_list, 0, LV_PART_MAIN);
    lv_obj_set_style_pad_gap(g_card_list, control_button_x_interval - control_button_width, LV_PART_MAIN);
    lv_obj_set_flex_flow(g_card_list, layout->is_large ? LV_FLEX_FLOW_ROW_WRAP : LV_FLEX_FLOW_ROW);
    lv_obj_set_flex_align(g_card_list, LV_FLEX_ALIGN_START, LV_FLEX_ALIGN_START, LV_FLEX_ALIGN_START);
    lv_obj_set_scroll_dir(g_card_list, layout->is_large ? LV_DIR_VER : LV_DIR_HOR);
    lv_obj_set_scrollbar_mode(g_card_list, LV_SCROLLBAR_MODE_OFF);
    lv_obj_clear_flag(g_card_list, LV_OBJ_FLAG_SCROLL_ELASTIC);
    lv_obj_add_event_cb(g_card_list, page_scroll_cb, LV_EVENT_SCROLL_BEGIN, NULL);
    lv_obj_add_event_cb(g_card_list, page_scroll_cb, LV_EVENT_SCROLL_END, NULL);

    lv_obj_t *btn_return = lv_btn_create(g_page);
    lv_obj_set_size(btn_return, ui_layout_scale(btn_return_width), ui_layout_scale(btn_return_width));
...
void ui_matter_ctrl_start(void (*fn)(void))
{
    ...
    lv_obj_set_size(g_page, layout->page_width, layout->page_height);
    lv_obj_set_style_pad_all(g_page, 0, LV_PART_MAIN);
    ...
    lv_obj_set_style_radius(g_page, ui_layout_scale(15), LV_STATE_DEFAULT);
    ...
    lv_obj_set_style_text_color(g_hint_label, lv_color_make(40, 40, 40), LV_STATE_DEFAULT);
    lv_obj_set_style_text_font(g_hint_label, layout->body_font, LV_STATE_DEFAULT);
    lv_obj_align(g_hint_label, LV_ALIGN_TOP_MID, 0,
                 ui_layout_scale(qrcode_align_y) +
                         LV_MIN(ui_layout_scale(qrcode_width), 220) + ui_layout_scale(6));
...
{
    const ui_layout_t *layout = ui_main_get_layout();
    lv_coord_t responsive_qr_width = LV_MIN(ui_layout_scale(qrcode_width), 220);
    if (!QRcode) {
        QRcode = lv_qrcode_create(g_page);
        lv_qrcode_set_size(QRcode, responsive_qr_width);
        lv_qrcode_set_dark_color(QRcode, lv_color_black());
        lv_qrcode_set_light_color(QRcode, lv_color_white());
        lv_obj_align(QRcode, LV_ALIGN_TOP_MID, 0, ui_layout_scale(qrcode_align_y));
    }
    ...
    if (!g_qr_text) {
        ...
        lv_obj_set_width(g_qr_text, LV_MIN(ui_layout_scale(250), layout->page_width - ui_layout_scale(40)));
        lv_label_set_long_mode(g_qr_text, LV_LABEL_LONG_WRAP);
        lv_obj_set_style_text_font(g_qr_text, layout->body_font, LV_STATE_DEFAULT);
        lv_obj_align(g_qr_text, LV_ALIGN_TOP_MID, 0, ui_layout_scale(qrcode_align_y + 8));
    }
